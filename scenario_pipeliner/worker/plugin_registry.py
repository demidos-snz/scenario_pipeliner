...
StateClass = type[Any]
BrokerHooks = Any
BrokerSettingsProvider = Callable[[], Any]
...
@dataclass(frozen=True, slots=True)
class BrokerPluginDefinition:
    plugin_name: str
    hooks: BrokerHooks
    settings_provider: BrokerSettingsProvider
...
class MainPipelinePluginRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._broker_plugins: dict[str, BrokerPluginDefinition] = {}
    ...
    def add_broker_hooks(
        ...
        *,
        ...
        if name in self._broker_plugins:
            raise ValueError(f"duplicate broker hooks for plugin: {name!r}")
        self._broker_plugins[name] = BrokerPluginDefinition(
            plugin_name=name,
            hooks=hooks,
    ...
    @property
    def broker_plugins(self) -> dict[str, BrokerPluginDefinition]:
        return dict(self._broker_plugins)
