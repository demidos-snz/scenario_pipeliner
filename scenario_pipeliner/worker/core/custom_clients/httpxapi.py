...
class AsyncHTTPxAPIClient(AsyncAPIClient[HTTPResponse, TAPISettings]):
    """Async HTTP client with auth and CRUD."""
    ...
    async def connect(self) -> None:
        """Open the HTTP session and authenticate when credentials are set."""
    ...
    async def authenticate(self) -> str:
        """Obtain an access token (OAuth2 password form or static API_TOKEN)."""
        if self.settings.API_TOKEN:
            self._access_token = self.settings.API_TOKEN
