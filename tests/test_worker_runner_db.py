...
def test_runner_db_empty_poll_is_debug(
    ...
    logger_name = "scenario_pipeliner.worker.execution.runner_db"
    with caplog.at_level(logging.DEBUG, logger=logger_name):
        asyncio.run(runner.execute())
    messages = [record.getMessage() for record in caplog.records]
    assert "No tasks found" in messages
    assert not any(
        record.levelno >= logging.WARNING and "No tasks found" in record.getMessage()
