...
def test_cli_plugin_checksum_write(tmp_path: Path) -> None:
    ...
    assert sha256_directory(plugin_dir) == expected
...
def test_checksum_ignores_manifest_checksum_value(tmp_path: Path) -> None:
    plugin_dir = _write_plugin(tmp_path, "demo_plugin", checksum_value="0" * 64)
...
def test_checksum_includes_manifest_policy_fields(tmp_path: Path) -> None:
    plugin_dir = _write_plugin(tmp_path, "demo_plugin", checksum_value="0" * 64)
    before = sha256_directory(plugin_dir)
    manifest_path = plugin_dir / "plugin.manifest.json"
    payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    payload["core_compat"] = ">=0.0.1,<1.0"
    manifest_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    assert sha256_directory(plugin_dir) != before
