...
def test_runner_app_from_env_loads_dotenv(tmp_path: Path, monkeypatch) -> None:
    import asyncio
    ...
    monkeypatch.delenv("SCENARIO_PIPELINER_CORE_VERSION", raising=False)
    (tmp_path / ".env").write_text(
        "SCENARIO_PIPELINER_CORE_VERSION=from-dotenv\n",
        encoding="utf-8",
    )
    captured: dict[str, str | None] = {}

    def _from_env():
        captured["v"] = os.environ.get("SCENARIO_PIPELINER_CORE_VERSION")
        raise RuntimeError("stop-after-dotenv")

    monkeypatch.setattr(
        "scenario_pipeliner.worker.runtime.app.setup_logging",
        lambda: None,
    )
    monkeypatch.setattr(
        "scenario_pipeliner.worker.runtime.app.RuntimeBootstrap.from_env",
        _from_env,
    )
    with pytest.raises(RuntimeError, match="stop-after-dotenv"):
        asyncio.run(RunnerApp.from_env())
    assert captured["v"] == "from-dotenv"
